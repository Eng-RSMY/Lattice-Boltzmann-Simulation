...
for t = 1:iterations
   ...
   % Relaxation/Collision {{{2
     ...
     % Outlet
     uu(end,1:end) = 2*uu(end-1,1:end) - uu(end-2,1:end);
     vv(end,1:end) = 2*vv(end-1,1:end) - vv(end-2,1:end);
     rho(end,1:end) = 2*rho(end-1,1:end) - rho(end-2,1:end);
%      uu(end,1:end) = (2*uu(end,1:end) + [uu(end,2:end) uu(end,end)] + [uu(end,1) uu(end,1:end-1)])/4;
%      vv(end,1:end) = (2*vv(end,1:end) + [vv(end,2:end) vv(end,end)] + [vv(end,1) vv(end,1:end-1)])/4;
%      rho(end,1:end) = (2*rho(end,1:end) + [rho(end,2:end) rho(end,end)] + [rho(end,1) rho(end,1:end-1)])/4;
     Pxx  = rho.*(cs^2 + uu.^2) -0*rho.*cs^2/2/beta .* 2.*(circshift(uu,[1 0])-circshift(uu,[2 0]));
     Pyy  = rho.*(cs^2 + vv.^2) -0*rho.*cs^2/2/beta .* 2.*(circshift(vv,[1 -1])-circshift(vv,[1 1]))/2;
     Pxy  = rho.*uu.*vv -0*rho.*cs^2/2/beta .* ((circshift(uu,[1 -1])-circshift(uu,[1 1]))/2 + (circshift(vv,[1 0])-circshift(vv,[2 0]))); % = Pyx
     uu(end-1,50)-uu(end-2,50)
     uu(end,50)
     for i = [4 7 8]
        fgr(i,:,:) = W(i) .* ...
            ( ...
