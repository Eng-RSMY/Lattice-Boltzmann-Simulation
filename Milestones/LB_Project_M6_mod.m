...
for t = 1:iterations
   ...
   % Relaxation/Collision {{{2
     ...
     Pxy  = reshape(((cx.*cy) * reshape(f,9,Nx*Ny)),Nx,Ny) ./ rho; % = Pyx
       rho(end,2:end-1) = 2*rho(end-1,2:end-1)-rho(end-2,2:end-1);
% %       rho(end,2:end-1) = (2*rho(end,2:end-1) + [rho(end,3:end-1) rho(end,end-1)] + [rho(end,2) rho(end,2:end-2)])/4;
%       uu(end,2:end-1)  = 2*uu(end-1,2:end-1)  - uu(end-2,2:end-1);
% %       uu(end,2:end-1) = (2*uu(end,2:end-1) + [uu(end,3:end-1) uu(end,end-1)] + [uu(end,2) uu(end,2:end-2)])/4;
%       vv(end,2:end-1)  = 2*vv(end-1,2:end-1)  - vv(end-2,2:end-1);
%       Pxx(end,2:end-1) = 2*Pxx(end-1,2:end-1) - Pxx(end-2,2:end-1);
%       Pyy(end,2:end-1) = 2*Pyy(end-1,2:end-1) - Pyy(end-2,2:end-1);
%       Pxy(end,2:end-1) = 2*Pxy(end-1,2:end-1) - Pxy(end-2,2:end-1);
     for i = [4 7 8]
        fgr(i,:,:) = W(i) .* ...
            ( ...
            + rho ...
   ...
   % Boundary Conditions {{{2
    ...
    for i = [4 7 8]
        %fold(i,end,2:end-1) = f(i,end,2:end-1);
%         f(i,end,2:end-1) = fgr(i,end,2:end-1);
     end
     % Domain
     for i = 1:9
